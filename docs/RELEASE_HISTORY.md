...
## 2026-09-15 — 正式库禁止自动生成演示/测试数据（云端污染防护）

类型：Prod 数据安全红线。

背景：

- 用户在手机端打开正式站点，登录后发现系统自动出现一批示例数据（示例待办 / 示例账目 /
  示例学习方向等），随后这批数据随「拉取-合并-推送」被合并上传到正式云库，
  用户不得不回滚云端历史版本。
- 根因：`app.js` 的「首次使用预置示例」块，触发条件包含「尚未登录（无 sessionPasscode）」。
  新设备（含手机 PWA）打开时必然处于未登录状态，且本地数据为空，于是必然预置示例数据；
  登录后 `cloudPull` 合并、`cloudPush` 回推，示例数据即进入云端。

修复：

1. 预置示例仅在 `TEST_BUILD`（测试版）生效，正式库永不自动生成任何数据；
   新设备打开后是干净空白状态，登录后以云端数据为准。
2. 自动预置的数据打上 `_demoSeeded` 标记；`cloudPush` 顶部新增红线：
   带该标记的本机状态一律拒绝上传，并提示用户清空本机数据。
3. 历史残留自愈：启动时若检出早期版本预置的「欢迎使用 / 已预置示例数据」备忘，
   自动补打标记阻止上传，并弹出提示引导清空本机数据后重新登录。
4. `node --check app.js` 通过；版本戳 `app.js?v=prod-no-demo-seed-20260915-1`；CHANGELOG 记 v3.21.4。
