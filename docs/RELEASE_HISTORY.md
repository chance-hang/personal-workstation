...
## 2026-09-15 — 账户名称跨设备同步（v3.21.5）

类型：功能增强。

背景：

- 账户名称属于本机账户配置（localStorage `lifeworkbench_sync_profiles`），此前改名只在当前
  设备生效，手机/其他设备仍显示旧名，容易被误认为同步 Bug。

方案：

- 把账户名称作为元信息 `__acct:{name,ts}` **随加密业务数据一起往返云端**：推送时附加，
  解密后**立即剥离**，绝不进入业务状态 `S` / 本地 data / 导出文件。
- 冲突裁决：按 `_acctNameTs` 时间戳 last-write-wins。本机改名更晚则保留本机（下次推送覆盖
  云端）；云端更晚则采用云端名并回写本机配置 + 刷新界面。
- 覆盖所有解密入口：cloudPull、cloudPush 的推送前预合并、历史版本恢复（同步回写）、
  历史版本预览（仅剥离不改名，避免预览动作修改本机配置）。
- 安全边界不变：登录账号与 GitHub Token 仍只存本机，不参与任何同步。
- 编辑账户弹窗补充说明文案；保存后立即触发一次推送，改名即时上云。

自测：`tools` 级纯逻辑用例 10 项全过（含旧版数据无 `__acct`、空名称、无当前账户等边界）。
