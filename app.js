...
const CHANGELOG=[
  ...
  {version:'v3.21.2',date:'2026-08-31',modules:['今日','账本'],status:'已上线',items:[
    '修复今日快捷入口定位被固定页头遮挡，待办与打卡模块会完整露出',
    '点击「＋记账」时固定进入记账视图，不再沿用上次停留的资金规划或分析视图'
  ]},
  {version:'v3.21.1',date:'2026-08-31',modules:['今日','账本','学习','我的'],status:'已上线',items:[
    '修复账本切换器初始化报错，恢复今日四个快捷按钮及全站首次渲染',
    '修复账本分析与资金规划高亮状态，并取消切换栏吸顶，避免滚动遮挡内容',
    '恢复学习内容、更新日志等依赖首次渲染的模块'
  ]},
  {version:'v3.21.0',date:'2026-08-31',modules:['数据安全','今日','账本','学习','布局'],status:'已上线',items:[
  ...
  {version:'v3.20.25',date:'2026-08-27',modules:['账本'],status:'已上线',items:[
  ...
  {version:'v3.20.24',date:'2026-08-27',modules:['账本'],status:'已上线',items:[
  ...
  {version:'v3.20.23',date:'2026-08-27',modules:['账本'],status:'已上线',items:[
    '账本新增「资金管理」模块：输入每月工资收入，系统自动计算资金分配',
  ...
  {version:'v3.20.22',date:'2026-08-26',modules:['学习'],status:'已上线',items:[
  ...
  {version:'v3.20.21',date:'2026-08-26',modules:['学习'],status:'已上线',items:[
  ...
  {version:'v3.20.20',date:'2026-08-26',modules:['学习','数据安全'],status:'已上线',items:[
  ...
  {version:'v3.20.19',date:'2026-08-26',modules:['学习'],status:'已上线',items:[
  ...
  {version:'v3.20.17',date:'2026-08-26',modules:['学习'],status:'已上线',items:[
  ...
  {version:'v3.20.16',date:'2026-08-26',modules:['学习'],status:'已上线',items:[
  ...
  {version:'v3.20.15',date:'2026-08-26',modules:['学习'],status:'已上线',items:[
  ...
  {version:'v3.20.14b',date:'2026-08-26',modules:['学习'],status:'已上线',items:[
  ...
  {version:'v3.20.14',date:'2026-08-26',modules:['学习'],status:'已上线',items:[
  ...
  {version:'v3.20.13',date:'2026-08-26',modules:['学习'],status:'已上线',items:[
  ...
  {version:'v3.20.12',date:'2026-08-26',modules:['学习'],status:'已上线',items:[
  ...
  {version:'v3.20.11',date:'2026-08-26',modules:['学习'],status:'已上线',items:[
  ...
  {version:'v3.20.10',date:'2026-08-26',modules:['学习'],status:'已上线',items:[
  ...
  {version:'v3.20.9',date:'2026-08-26',modules:['学习'],status:'已上线',items:[
  ...
  {version:'v3.20.6',date:'2026-08-25',modules:['学习'],items:[
    ...
    '新增：「大纲」页一键导出大纲，按当前顺序输出 2 空格缩进文本（含主题/知识点 + #标签），便于维护大纲后导入当前工作台',
  ],status:'已上线'},
  ...
  {version:'v3.20',date:'2026-08-25',modules:['学习'],items:[
    ...
    '导入支持 #标签 解析，导入前可先预览树形结果，确认后再写入工作台数据'
  ],status:'已上线'},
  ...
  {version:'v3.9.1',date:'2026-08-12',modules:['云同步'],items:['同步防丢数据修复（与 v3.8.1 同源）：改为按 id 合并同步，本地已有记录永不删除','推送失败指数退避自动重试（5s→10s→…上限60s）','页面切后台/关闭时强制 flush 本地未上云数据','「立即同步」改为先推本地再拉云合并的双向同步','本版本同时含「学习」大模块（v3.9）'],status:'已上线'},
  {version:'v3.9',date:'2026-08-12',modules:['学习'],items:['新增「学习」大模块：自定义学习方向与内容（方向下挂「资料 / 子主题」二级）','UX 重构为「方向即主页」：取消顶部 6 Tab，改为方向卡片展开，一个卡片内直接切换计划/内容/问题/知识库/复习，减少来回切换','支持一键快速添加：方向内直接输入标题新建计划（带模板）/ 内容 / 问题，无需先切 Tab 再进弹窗','计划支持模板快速创建：21 天入门 / 读书笔记 / 项目实战，自动生成阶段与任务','问题状态支持点击徽章单键循环切换（未解决 → 解决中 → 已解决）','已解决问题可一键加入复习，按 1/3/7/16/30 天间隔递增提醒','手动制定学习计划：方向 → 阶段 → 任务 → 截止日，自动计算完成进度','已解决问题与答案自动沉淀为「知识库」，可搜索','学习数据随导出 / 导入 / 合并导入 / 云同步自动携带'],status:'已上线'},
  {version:'v3.7',date:'2026-08-10',modules:['账本'],items:[
  ...
  {version:'v3.8',date:'2026-08-12',modules:['历史信息'],items:['历史信息新增「历史日报」视图：在「历史」页切换「历史日报」，按日期区间列出有记录每一天','每天支持「复制日报」一键复制当天工作日报，或「查看」在弹窗中编辑 / 下载 .md','新增「复制本区间全部日报」，可批量复制所选区间所有日报','日期区间筛选栏置于页签上方，时间线记录与历史日报共享同一套筛选条件'],status:'已上线'},
  {version:'v3.6',date:'2026-08-10',modules:['历史信息','工作日志','账本'],items:[
  ...
  {version:'v3.5',date:'2026-08-06',modules:['更新日志','模块管理'],items:['新增“更新日志”模块，用于沉淀每次迭代的升级功能清单','记录版本号、更新日期、涉及模块与功能点，前端可直接查看','更新日志模块纳入“编辑模块”，可显示/隐藏和排序'],status:'已上线'},
  ...
  {version:'v3.3',date:'2026-08-06',modules:['模块管理'],items:['新增“编辑模块”入口','支持模块显示/隐藏、上下排序、恢复默认','模块配置保存在浏览器 localStorage'],status:'已上线'},
...
function load(){
  try{
    ...
    if(!d.learning||typeof d.learning!=='object')d.learning={directions:[],contents:[],plans:[],questions:[],reviews:[],mistakes:[],selftests:[]};
    ['directions','contents','plans','questions','reviews','mistakes','selftests'].forEach(k=>{if(!Array.isArray(d.learning[k]))d.learning[k]=[]});
    if(!d.habitArchive||typeof d.habitArchive!=='object')d.habitArchive={};
    if(!d.customCats||typeof d.customCats!=='object')d.customCats={expense:[],income:[]};
    if(!Array.isArray(d.customCats.expense))d.customCats.expense=[];
    if(!Array.isArray(d.customCats.income))d.customCats.income=[];
    if(!d.budgetCats||typeof d.budgetCats!=='object')d.budgetCats={};
    if(!Array.isArray(d.quickNotes))d.quickNotes=[];
    if(!d.updatedAt)d.updatedAt=Date.now();
    return d;
  }catch(e){return structuredClone(DEFAULT)}
}
const IDB_NAME='lifeworkbench_local_v1',IDB_STORE='snapshots';
let mirrorTimer=null,mirrorState='等待写入',lastLocalSaveError='';
function stateJson(){return JSON.stringify(S)}
function fmtBytes(n){if(!Number.isFinite(n))return '—';if(n<1024)return n+' B';if(n<1048576)return (n/1024).toFixed(n<10240?1:0)+' KB';return (n/1048576).toFixed(2)+' MB'}
function openLocalMirror(){return new Promise((resolve,reject)=>{if(!window.indexedDB){reject(new Error('浏览器不支持 IndexedDB'));return}const q=indexedDB.open(IDB_NAME,1);q.onupgradeneeded=()=>{if(!q.result.objectStoreNames.contains(IDB_STORE))q.result.createObjectStore(IDB_STORE,{keyPath:'slot'})};q.onsuccess=()=>resolve(q.result);q.onerror=()=>reject(q.error||new Error('镜像数据库打开失败'))})}
async function readLocalMirror(slot='current'){const db=await openLocalMirror();return new Promise((resolve,reject)=>{const q=db.transaction(IDB_STORE,'readonly').objectStore(IDB_STORE).get(slot);q.onsuccess=()=>{db.close();resolve(q.result||null)};q.onerror=()=>{db.close();reject(q.error)}})}
async function writeLocalMirror(json,manual=false){try{const db=await openLocalMirror();await new Promise((resolve,reject)=>{const tx=db.transaction(IDB_STORE,'readwrite'),st=tx.objectStore(IDB_STORE),get=st.get('current');get.onsuccess=()=>{if(get.result)st.put({...get.result,slot:'previous'});st.put({slot:'current',json,updatedAt:S.updatedAt||Date.now(),savedAt:Date.now(),manual});};tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)});db.close();mirrorState='正常';lastLocalSaveError='';renderStorageHealth()}catch(e){mirrorState='失败';lastLocalSaveError=e.message||String(e);renderStorageHealth()}}
function queueLocalMirror(json){clearTimeout(mirrorTimer);mirrorState='待同步';mirrorTimer=setTimeout(()=>writeLocalMirror(json),700)}
function save(){const json=stateJson();let ok=true;try{localStorage.setItem(KEY,json)}catch(e){ok=false;lastLocalSaveError='localStorage 写入失败：'+(e.message||e);toast('本地存储空间不足，已转存安全镜像')}queueLocalMirror(json);renderStorageHealth();return ok}
function persist(){S.updatedAt=Date.now();save();schedulePush()}
async function renderStorageHealth(){const a=$('#storageDataSize'),b=$('#storageQuota'),c=$('#storageMirror'),n=$('#storageHealthNote');if(!a||!b||!c)return;let json='';try{json=stateJson()}catch(e){}a.textContent=fmtBytes(new Blob([json]).size);c.textContent=mirrorState;if(n)n.textContent=lastLocalSaveError||'现有 localStorage 保持兼容；每次修改会异步镜像到容量更大的 IndexedDB，并保留当前与上一代快照。';if(navigator.storage&&navigator.storage.estimate){try{const e=await navigator.storage.estimate();b.textContent=fmtBytes(e.usage||0);b.title='预计可用 '+fmtBytes(e.quota||0)}catch(e){b.textContent='未知'}}else b.textContent='不支持'}
async function createLocalSnapshot(){mirrorState='写入中';renderStorageHealth();await writeLocalMirror(stateJson(),true);toast(mirrorState==='正常'?'本机安全快照已创建':'快照创建失败')}
async function restoreLocalSnapshot(){try{const snap=await readLocalMirror('current');if(!snap||!snap.json){toast('还没有可恢复的本机镜像');return}const when=new Date(snap.savedAt||snap.updatedAt).toLocaleString('zh-CN');modal(`<h4>从本机镜像恢复<span class="modal-close" onclick="closeModal()">×</span></h4><p style="color:var(--text-2)">将恢复 ${when} 的快照。当前数据会先写入上一代镜像，确认继续？</p><div class="btns"><button class="cancel" onclick="closeModal()">取消</button><button class="ok" id="mirrorRestoreGo">恢复</button></div>`,{noMaskClose:true});$('#mirrorRestoreGo').onclick=()=>{try{const d=JSON.parse(snap.json);writeLocalMirror(stateJson(),true);S=Object.assign(structuredClone(DEFAULT),d);if(Array.isArray(d.modules))restoreModules(d.modules);persist();closeModal();renderAll();toast('已从本机镜像恢复')}catch(e){toast('镜像数据无法读取')}}}catch(e){toast('读取本机镜像失败')}}
...
const SUBMODULES={
  ...
  me:[
    ...
    {id:'lnOrganize',label:'学习资料整理',ic:'M3 6h18M3 12h18M3 18h18'},
...
/* ============ 我的 ============ */
$('#lnOrganizeBtn').onclick=lnOrganizeConfirm;
$('#lnExportBtn').onclick=lnExportLearning;
$('#exportBtn').onclick=()=>{
...
function openImportPaste(){
  modal(`<h4>粘贴 JSON 覆盖导入<span class="modal-close" onclick="confirmClose(()=>{$('#ipGo').click()})">×</span></h4>
    <p class="muted" style="margin:0 0 10px;font-size:12.5px">粘贴 JSON 内容，将<b>覆盖</b>当前全部数据，不可恢复。</p>
...
function confirmOverwrite(d){
  modal(`<h4>确认导入<span class="modal-close" onclick="closeModal()">×</span></h4><p style="color:var(--text-2)">导入将<b>覆盖</b>当前全部数据，且不可恢复。确定继续？</p>
    <div class="btns"><button class="cancel" onclick="closeModal()">取消</button><button class="ok" id="impOk" style="background:var(--bad)">覆盖导入</button></div>`,{noMaskClose:true});
  $('#impOk').onclick=()=>{S=Object.assign(structuredClone(DEFAULT),d);if(Array.isArray(d.modules))restoreModules(d.modules);persist();closeModal();renderAll();toast('导入成功')};
}
...
$('#clearBtn').onclick=()=>{
  ...
  ok.onclick=()=>{S=structuredClone(DEFAULT);persist();closeModal();renderAll();toast('已清空全部')};
};
...
function loadProfiles(){
  try{
    ...
    if(raw){
      ...
      if(!migrated)syncProfiles=[JSON.parse(JSON.stringify(DEFAULT_PROFILE))];
...
function setSyncStatus(state){
  ...
  const map={off:['未开启','off'],sync:['同步中','sync'],ok:['已同步','on'],err:['同步失败','err']};
...
function mergeState(remote){
  let changed=false;
  const unionArr=(loc,rem)=>{
    const map=new Map();
    (loc||[]).forEach(x=>{if(x&&x.id)map.set(x.id,x)});
    (rem||[]).forEach(x=>{if(x&&x.id&&!map.has(x.id)){map.set(x.id,x);changed=true}});
    return [...map.values()];
  };
  /* 字段级并集合并：同 id 时「远端非空字段」覆盖本地（避免 unionArr 整条不更新导致学习心得/答案等不同步）
     —— 用法：先 unionArr 把 id 补齐，再按字段并集刷内容；createdAt 取两端较早者（防新建端截断） */
  const mergeItemsFields=(locArr,remArr,fields)=>{
    const rMap=new Map();
    (remArr||[]).forEach(x=>{if(x&&x.id)rMap.set(x.id,x)});
    locArr.forEach(item=>{
      ...
      fields.forEach(f=>{
        const rv=r[f];
        const isArr=Array.isArray(rv);
        const rvEmpty=rv==null||rv===''||(isArr&&rv.length===0);
        if(!rvEmpty && JSON.stringify(rv)!==JSON.stringify(item[f])){ item[f]=rv; touched=true; }
      });
      if(r.createdAt && (!item.createdAt || r.createdAt<item.createdAt)){ item.createdAt=r.createdAt; touched=true; }
      if(touched) changed=true;
    });
  };
  /* 习惯合并：同 id 习惯保留本地属性（名称/图标/颜色等可能刚改过），但 history 深度合并取并集
     ——避免「本地空 history 覆盖远端已打卡记录」导致打卡数据跨设备丢失 */
  const delIds=new Set(Object.keys(S.habitArchive||{}));
  S.todos=unionArr(S.todos,remote.todos);
  (function(){
    const map=new Map();
    (S.habits||[]).forEach(x=>{if(x&&x.id)map.set(x.id,x)});
    (remote.habits||[]).forEach(x=>{
      if(!x||!x.id||delIds.has(x.id))return;
      if(map.has(x.id)){
        /* 同 id：保留本地属性，history 深度合并取并集；createdAt 取两端较早者（避免本地较新 createdAt 截断连续天数） */
        const loc=map.get(x.id);
        const lh=loc.history||{}, rh=x.history||{};
        let merged=false;
        Object.keys(rh).forEach(d=>{ if(!(d in lh)){ lh[d]=rh[d]; merged=true; } });
        if(merged){ loc.history=lh; changed=true; } /* 关键回写：修「本地 history 为 undefined 时引用丢失」导致历史被清空 */
        if(x.createdAt && (!loc.createdAt || x.createdAt<loc.createdAt)){ loc.createdAt=x.createdAt; changed=true; }
      }else{
        map.set(x.id,x);changed=true;
      }
    });
    S.habits=[...map.values()];
  }());
  S.ledger=unionArr(S.ledger,remote.ledger);
  /* 自定义快捷备注（v3.18）：按名称并集合并，任一端添加即同步 */
  if(Array.isArray(remote.quickNotes)){
    const qnSet=new Set(S.quickNotes||[]);
  ...
  if(remote.learning&&S.learning){
    /* 7 个 key 全过（修复之前 unionArr 漏 selftests；mistakes 是派生暂留空不影响） */
    ['directions','contents','plans','questions','reviews','mistakes','selftests'].forEach(k=>{
      if(Array.isArray(remote.learning[k])) S.learning[k]=unionArr(S.learning[k],remote.learning[k]);
    });
    /* 字段级深度合并（同 id 时远端非空字段覆盖本地）—— 修复学习心得/答案/详情等不同步
       plans 嵌套复杂（milestones>tasks），保留 unionArr 即可 */
    mergeItemsFields(S.learning.contents,remote.learning.contents,['name','kind','note','notes','notesSymbol','url','directionId','nodeId','parentId','tags','mastered']);
    mergeItemsFields(S.learning.questions,remote.learning.questions,['title','detail','status','answer','tags','hard','solvedAt','directionId','contentId','nodeId']);
    mergeItemsFields(S.learning.directions,remote.learning.directions,['name','description','active','color']);
    mergeItemsFields(S.learning.reviews,remote.learning.reviews,['refType','refId','directionId','nodeId','due','done','note']);
    mergeItemsFields(S.learning.selftests,remote.learning.selftests,['title','score','weak','note','directionId','nodeId']);
  }
  if(remote.moods&&typeof remote.moods==='object'){
    S.moods=S.moods||{};
    Object.keys(remote.moods).forEach(d=>{ if(!(d in S.moods)){S.moods[d]=remote.moods[d];changed=true;} });
  }
  /* 模块显示/隐藏配置也要随云同步：以「任一端显示即显示」的并集合并，避免手机端开启模块后电脑端拉取时丢失 */
  if(remote.modules&&Array.isArray(remote.modules)){
    const rem=normalizeModules(remote.modules);
    const remMap={};rem.forEach(c=>remMap[c.id]=c.visible);
    const merged=moduleCfg.map(c=>({id:c.id,visible:c.visible||(c.id in remMap?remMap[c.id]:false)}));
    rem.forEach(c=>{ if(!merged.some(x=>x.id===c.id)) merged.push({id:c.id,visible:c.visible}); });
    const modChanged=merged.length!==moduleCfg.length||merged.some((c,i)=>moduleCfg[i]&&c.visible!==moduleCfg[i].visible);
    if(modChanged){ moduleCfg=merged; S.modules=merged; try{localStorage.setItem(MODULE_KEY,JSON.stringify(merged))}catch(e){} changed=true; applyModuleLayout(); }
  }
  /* 已删除习惯的归档历史也要跨设备合并，确保任一端删除习惯后其历史记录仍保留 */
  if(remote.habitArchive&&typeof remote.habitArchive==='object'){
    S.habitArchive=S.habitArchive||{};
    Object.keys(remote.habitArchive).forEach(id=>{
      const r=remote.habitArchive[id];
      if(!S.habitArchive[id]){ S.habitArchive[id]=r; changed=true; }
      else{
        const lh=S.habitArchive[id].history||{}, rh=(r&&r.history)||{};
        Object.keys(rh).forEach(ds=>{ if(!(ds in lh)){ lh[ds]=rh[ds]; changed=true; } });
      }
    });
  }
  return changed;
...
async function cloudPull(opts={}){
  ...
  syncBusy=true;setSyncStatus('sync');
  try{
    const r=await fetch(GITHUB_API+'/'+p.binId,{headers:GH_HEADERS(p.masterKey)});
    if(!r.ok)throw new Error('HTTP '+r.status);
    const data=await r.json();
...
function normalizeState(d){
  if(!d.worklog)d.worklog=[];if(!d.notes)d.notes=[];if(!d.countdowns)d.countdowns=[];if(!d.moods)d.moods={};if(!d.habits)d.habits=[];if(!d.todos)d.todos=[];if(!d.ledger)d.ledger=[];if(!d.learning||typeof d.learning!=='object')d.learning={directions:[],contents:[],plans:[],questions:[],reviews:[],mistakes:[],selftests:[]};['directions','contents','plans','questions','reviews','mistakes','selftests'].forEach(k=>{if(!Array.isArray(d.learning[k]))d.learning[k]=[]});
  if(!Array.isArray(d.quickNotes))d.quickNotes=[];
  return d;
}
...
async function fetchGistRevision(version){
  ...
  const r=await fetch(GITHUB_API+'/'+p.binId+'/'+version,{headers:GH_HEADERS(p.masterKey)});
  if(!r.ok)throw new Error('HTTP '+r.status);
  return await r.json();
}
...
async function fetchAllGistCommits(){
  ...
  let page=1, all=[];
  while(true){
    const r=await fetch(GITHUB_API+'/'+p.binId+'/commits?per_page='+per+'&page='+page,{headers:GH_HEADERS(p.masterKey)});
    if(!r.ok)throw new Error('HTTP '+r.status);
...
function promptLogin(profile){
  ...
  modal(`<h4>登录云同步账户<span class="modal-close" onclick="closeModal()">×</span></h4>
    ...
    <div class="btn-pair">
      ...
      <button id="loginOk" style="flex:1;background:var(--text);color:#fff;border-radius:var(--r-sm);padding:11px;font-weight:600">登录并同步</button>
    </div>`,{noMaskClose:true});
...
function renderAccountManager(){
  ...
  modal(`<h4>云同步 · 账户管理<span class="modal-close" onclick="closeModal()">×</span></h4>
    ...
    <button id="addAccBtn" class="acc-add">+ 新增账户</button>
...
/* —— 学习资料整理工具（v3.20.20） —— */
function lnExportLearning(){
  const L0=lnData();
  const data={exportedAt:new Date().toISOString(),learning:JSON.parse(JSON.stringify(L0))};
  const json=JSON.stringify(data,null,2);
  const blob=new Blob([json],{type:'application/json'});
  const ts=new Date().toISOString().replace(/[-:T]/g,'').slice(0,14);
  const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='学习资料备份-'+ts+'.json';document.body.appendChild(a);a.click();a.remove();
  setTimeout(()=>URL.revokeObjectURL(a.href),1000);
  toast('已导出学习资料备份');
}
function lnOrganizeTree(){
  const L0=lnData();
  if(!L0.contents||!L0.contents.length){toast('没有学习资料');return;}
  // 0) 安全：运行前自动导出「整理前备份」
  try{
    const data={exportedAt:new Date().toISOString(),learning:JSON.parse(JSON.stringify(L0))};
    const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'});
    const ts=new Date().toISOString().replace(/[-:T]/g,'').slice(0,14);
    const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='整理前备份-'+ts+'.json';document.body.appendChild(a);a.click();a.remove();
    setTimeout(()=>URL.revokeObjectURL(a.href),1000);
  }catch(e){/* 备份失败不阻断整理 */}
  const byId={};L0.contents.forEach(c=>byId[c.id]=c);
  let fixedParent=0,fixedCycle=0;
  // 1) 修复 parentId 完整性（基于原始快照，避免修改自身影响后续判定）
  const origParent={};L0.contents.forEach(c=>origParent[c.id]=c.parentId||null);
  L0.contents.forEach(c=>{
    const pid=origParent[c.id];
    if(!pid)return;
    const p=byId[pid];
    if(!p){c.parentId=null;fixedParent++;return;}
    if((p.directionId||null)!==(c.directionId||null)){c.parentId=null;fixedParent++;return;}
    // 成环检测：pid 是否为 c 的后代
    let cur=p,depth=0,cyc=false;
    while(cur&&depth<300){if(cur.id===c.id){cyc=true;break;}cur=origParent[cur.id]?byId[origParent[cur.id]]:null;depth++;}
    if(cyc){c.parentId=null;fixedCycle++;}
  });
  // 2) 按 (directionId, parentId) 兄弟组，依 createdAt 升序重建 order
  let fixedOrder=0;
  const groups={};
  L0.contents.forEach(c=>{const key=(c.directionId||'__n__')+'|'+(c.parentId||'__root__');(groups[key]=groups[key]||[]).push(c);});
  Object.keys(groups).forEach(k=>{
    const g=groups[k];
    g.sort((a,b)=>{
      const ak=a.createdAt||'',bk=b.createdAt||'';
      if(ak&&bk)return ak.localeCompare(bk)||String(a.id).localeCompare(String(b.id));
      if(ak&&!bk)return -1;if(!ak&&bk)return 1;
      return String(a.id).localeCompare(String(b.id));
    });
    g.forEach((c,i)=>{if(c.order!==i){c.order=i;fixedOrder++;}});
  });
  persist();lnRefresh();
  toast('已整理：顺序 '+fixedOrder+' 条 · 上下级 '+fixedParent+' 条'+(fixedCycle?(' · 成环 '+fixedCycle+' 条'):''));
}
function lnOrganizeConfirm(){
  modal('<h4>整理学习资料树<span class="modal-close" onclick="closeModal()">×</span></h4>'
    +'<p class="muted" style="margin:0 0 10px;font-size:12.5px">将按<b>创建时间</b>重建每个父子层级的顺序（修复倒序/乱序），并修复<b>悬空、跨方向、成环</b>的父子关系。运行前会自动导出一份「整理前备份」。</p>'
    +'<div class="btns"><button class="cancel" onclick="closeModal()">取消</button><button class="ok" id="lnOrgGo" style="background:var(--ok)">开始整理</button></div>',{noMaskClose:true});
  $('#lnOrgGo').onclick=()=>{closeModal();lnOrganizeTree()};
}
/* —— 问题 —— */
