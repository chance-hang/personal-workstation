...
const CHANGELOG=[
  ...
  {version:'v3.9.1',date:'2026-08-12',modules:['云同步'],items:['同步防丢数据修复（与正式版 v3.8.1 同源）：改为按 id 合并同步，本地已有记录永不删除','推送失败指数退避自动重试（5s→10s→…上限60s）','页面切后台/关闭时强制 flush 本地未上云数据','「立即同步」改为先推本地再拉云合并的双向同步','注：本测试版同时含「学习」大模块（v3.9-test）'],status:'已上线'},
  ...
  {version:'v3.8-test',date:'2026-08-12',modules:['历史信息'],items:['历史信息新增「历史日报」视图：在「历史」页切换「历史日报」，按日期区间列出有记录每一天','每天支持「复制日报」一键复制当天工作日报，或「查看」在弹窗中编辑 / 下载 .md','新增「复制本区间全部日报」，可批量复制所选区间所有日报','日期区间筛选栏置于页签上方，时间线记录与历史日报共享同一套筛选条件'],status:'已上线'},
  ...
  {version:'v3.5-test',date:'2026-08-06',modules:['更新日志','模块管理'],items:['新增“更新日志”模块，用于沉淀每次迭代的升级功能清单','记录版本号、更新日期、涉及模块与功能点，前端可直接查看','更新日志模块纳入“编辑模块”，可显示/隐藏和排序'],status:'已上线'},
  ...
  {version:'v3.3-test',date:'2026-08-06',modules:['模块管理'],items:['新增测试版标识和“编辑模块”入口','支持模块显示/隐藏、上下排序、恢复默认','模块配置保存在浏览器 localStorage'],status:'已上线'},
